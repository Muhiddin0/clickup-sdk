...
from clickup.savdo.when_broker_set.components import create_broker_message, create_broker_keyboard
from clickup_sdk.webhook import CustomFieldFilter, custom_field_set, custom_field_removed, WebhookEvent
...
async def handle_broker_set(event: WebhookEvent) -> None:
    ...
    for item in event.history_items:
        ...
        try:
            ...
            if not telegram_id:
                logger.warning(f"⚠️ No telegram_id found for broker task {relation_task_id}")
                continue
            ...
            logger.error(f"❌ Error processing broker task {relation_task_id}: {e}", exc_info=True)
...
@dispatcher.on("taskUpdated", CustomFieldFilter(field_name="Broker", on_set=False, on_remove=False, on_update=True))
